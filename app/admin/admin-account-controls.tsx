...
export function AdminAccountControls({ users }: { users: AdminUser[] }) {
  ...
  // often too much for a one-off goodwill grant (site owner: exactly this
  // complaint, wanted to hand out a few days instead).
  const [unit, setUnit] = useState<"months" | "days">("months");
  const [message, setMessage] = useState("");
  async function save(clear = false) {
    setMessage("Сохраняем…");
    const duration = clear ? 0 : Number(amount) || 0;
    const response = await fetch("/api/admin/accounts", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      email, planId: clear ? "trial" : planId,
      months: unit === "months" ? duration : undefined,
  ...
  return <section className="admin-block admin-account-controls">
    ...
    {!users.length ? <p className="admin-muted">Пользователей пока нет.</p> : <div className="admin-controls-grid">
      ...
      <label>Продлить на<div className="admin-duration-field">
        ...
        <select value={unit} onChange={(event) => setUnit(event.target.value === "days" ? "days" : "months")}>
