...
function coreRulesFor(goal: AdaptationGoal): readonly string[] {
  ...
  return [
    ...
    "Если использовал сведения из web_research, укажи в editorial_comment только названия источников (без полного URL) для редакторской проверки — не перечисляй все переданные web_research; в title, subtitle, body и SEO-полях ссылок быть не должно.",
