...
async function runContentPlanGeneration(input: ReturnType<typeof normalizePayload>, ownerEmail: string) {
  const instructions = [
      ...
      "Используй веб-поиск, чтобы опираться на реальные формулировки запросов и подтемы этой ниши, а не только на входной запрос и его парафразы. Достаточно 1-2 раундов поиска в начале работы: собери нужный контекст один раз и переходи к финальному плану — не чередуй поиск и промежуточные сообщения снова и снова, это тратит бюджет ответа впустую и не даёт задаче завершиться.",
      "Не пиши промежуточные текстовые сообщения о ходе работы («приступаю к поиску», «этого достаточно, перехожу к плану» и т.п.) между шагами. Единственный текстовый ответ — финальный JSON с готовым планом.",
