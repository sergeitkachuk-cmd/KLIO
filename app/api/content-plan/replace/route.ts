...
const FORMATS = ["SEO‑статья", "Экспертный разбор", "FAQ", "Сравнение", "Кейс", "Посадочная страница", "Рекламный текст", "Пост"] as const;
...
export async function POST(request: Request) {
  try {
    ...
    const instructions = [
      ...
      "Учитывай пожелание пользователя. Если выбрана смена ракурса — меняй смысловой угол; если продающий вариант — усиливай коммерческую задачу без давления; если новый формат — действительно меняй формат публикации. Во всех остальных случаях формат альтернативы должен совпадать с форматом заменяемой темы — не подменяй, например, «Пост» на «SEO‑статью», если пользователь не просил сменить формат.",
