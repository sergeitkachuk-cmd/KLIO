    ...
    try {
      const materialCall = await callAiModel<Record<string, unknown>>({
        ...
        instructions: [
          ...
          "Использованные факты из web_research отметь в editorial_comment коротко — названием источника без полного URL и пометкой «найдено в вебе, требует проверки». Не перечисляй все переданные web_research отдельным списком.",
