...
async function runMaterialGeneration(input: ReturnType<typeof normalizePayload>, ownerEmail: string, jobId: string) {
  ...
  const call = await callAiModel<Record<string, unknown>>({
    ...
    instructions: [
      ...
      "editorial_comment — 2–4 коротких предложения: использованный ракурс и, если действительно брал факты из web_research, только они — с названием источника без полного URL. Не перечисляй все переданные web_research отдельным списком и не добавляй общие оговорки о характере проверки источников. В публикацию не вставляй технические ссылки, если формат не предполагает список источников.",
