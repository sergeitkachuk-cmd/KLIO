...
// Polled by the client every few seconds while a material-generation job
// (started via POST /api/generate, which now only returns a jobId) runs in
// the background — see app/api/_lib/async-jobs.ts and the identical
// content-plan/status/route.ts for why the actual AI call isn't kept
// inside that first request anymore.
export async function GET(request: Request) {
  try {
    ...
    if (!job || job.kind !== "material_generation") {
    ...
    if (job.status === "failed") {
      return Response.json({ status: "failed", error: job.errorMessage || "Не удалось сформировать материал." });
    }
    ...
    console.error("generate status check failed", error);
